    ...
    <View className="flex-1 items-center pt-16 w-full">
      <View className="flex-1 w-full max-w-sm justify-center gap-10 px-4">
        <View>
          ...
          <Text className="text-white/40 text-lg text-center mt-3 leading-relaxed font-medium">
        ...
        <View className="items-center gap-6">
          <View className="bg-[#1A1A1A] rounded-3xl border border-white/5 p-5 w-full">
            <Text className="text-[10px] text-white/30 uppercase tracking-widest font-bold mb-4 text-center">
              Your Week
            </Text>
              ...
              {DAYS.map((day, i) => {
                ...
                return (
                  <View key={i} className="items-center gap-2">
                    <Text className="text-[11px] font-bold text-white/30">{day}</Text>
                    <View
                      className={`w-10 h-10 rounded-full items-center justify-center ${
                        skipped ? 'bg-white/5' : 'border border-dashed border-white/10'
                      }`}
                    >
                      {skipped && <X size={14} color="rgba(255,255,255,0.15)" />}
                    </View>
                  </View>
                );
              })}
            ...
            <View className="mt-5 items-center gap-2">
              <Dumbbell size={20} color="rgba(255,255,255,0.12)" />
              <Text className="text-[11px] text-white/20 font-medium">0 of 7 workouts</Text>
            </View>
          </View>
